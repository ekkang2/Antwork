...
2. **협업자 추가 및 삭제** 
    - 해당 프로젝트에 협업자를 초대하여 웹 소켓(Websocket Stomp) 방송을 통해 실시간으로 공유 및 수정 가능
    - 협업자를 초대하면 웹 소켓을 사용해서 실시간 알림 전송 가능
    - 로그인한 사용자의 토큰에 회사 요금제 타입을 실어서 무료/유료 회원에 따라 맞는 서비스를 제공할 수 있도록 구현
3. **프로젝트 작업 상태 관리**
    - 해당 프로젝트의 작업 상태(Todo, Ready, Doing, Done 등)를 등록/수정/삭제 가능하도록 구현
4. **프로젝트 작업 관리**
    - 해당 프로젝트의 작업 내용(작업명, 작업 내용, 작업 우선순위, 작업 크기, 작업 담당자)을 등록/수정/삭제 가능하도록 구현
    - 작업 담당자는 협업자 리스트 중에서 선택할 수 있도록 구현
    - React Beautiful DnD 라이브러리를 사용해서 작업에 대한 작업 상태를 드래그앤드랍 수정이 가능하도록 구현
5. **관리자 프로젝트 설정** 
    - 관리자가 프로젝트의 작업 우선순위와 크기 데이터를 동적으로 설정할 수 있도록 구현
